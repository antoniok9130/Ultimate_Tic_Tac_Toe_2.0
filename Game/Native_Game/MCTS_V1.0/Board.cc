...
unique_ptr<Quadrant> make_Quadrant() {
    unique_ptr<Quadrant> quadrant = make_unique<Quadrant>();
    for (auto& e : *quadrant) {
        e = 0;
    }
    return quadrant;
}
unique_ptr<Board2D> make_Board2D() {
    unique_ptr<Board2D> board = make_unique<Board2D>();
...
Quadrant make_Quadrant(Quadrant& other) {
    Quadrant quadrant;
    For(0, 9) { quadrant[i] = other[i]; }
    return quadrant;
}
Board2D make_Board2D(Board2D& other) {
    Board2D board;
    For(0, 9) {
        Forj(0, 9) { board[i][j] = other[i][j]; }
    }
    return board;
}
