...
Node* AI_move(Node* node, int iterations, std::ostream& logout=cout) {  // double thinkingTime
    logout << "    Player " << (node->getPlayer()) << " getting move" << endl;
    clock_t begin = clock();
    Move move = getMove(node, iterations - node->getNumVisits());  // thinkingTime
    clock_t end = clock();
    logout << "Took " << (double(end - begin) / CLOCKS_PER_SEC) << " seconds to get move" << endl;
    ...
    logout << "    Length:  " << node->getLength() << endl;
    logout << "    Winner:  " << node->getWinner() << endl;
...
void selfPlay(Node* game, std::ostream& logout, std::ostream& recordout) {
    ...
    try {
        while (game->getWinner() == N) {
            ...
            game = AI_move(game, 1000000, logout);
        }
...
void iterateSelfPlay(std::ostream& log = cout, std::ostream& record = cout) {
    ...
    try {
        ...
        if (analysis) {
            ...
            if (logPath != "cout"){
                ...
                std::ofstream record;
                record.open(recordPath, std::ios_base::app);

                iterateSelfPlay(log, record);
            }
            else{
                iterateSelfPlay();
            }
