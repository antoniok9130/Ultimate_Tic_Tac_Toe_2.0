...
class Q_Model(torch.nn.Module):
    ...
    def getMove(self, previousMove, quadrants, board):
        ...
        if random.random() < self.exploreProb:
            ...
            rewards = self.predict(extract_features(board))
            np.multiply(rewards, legal_moves, rewards)
            move = np.random.choice(np.nonzero(rewards == rewards.max()))

        return int(move//9), int(move%9)


def compute_reward(quadrants, board, winner, move, player, length, q):
    winner, time_step = simulation(quadrants, board, winner, move, player, length, q.getMove)

    return (1 if winner == player else -1)*(q.gamma**time_step)
